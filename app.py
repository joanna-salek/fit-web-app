...
def register():
    ...
    if not e_mail or not s_psw or not c_psw or not f_name or not l_name or not b_day:
        ...
        return redirect(url_for("go_to_register"))
...
def log_in():
    ...
    email_from_form = c.execute('''SELECT email FROM users WHERE email=?''', (email,)).fetchone()
    psw_from_form = c.execute('''SELECT psw FROM users WHERE email=?''', (email,)).fetchone()
...
@app.route('/BMI_form')
def bmi_form():
    return render_template("BMI-form.html")
...
@app.route('/orm-form')
def orm_form():
    return render_template("orm-form.html")
...
def orm():
    ...
    orm = round(orm * 2) / 2
    return render_template("orm.html", orm=orm)
